...
Jira ticket (label=sonarqube + sonar-hash:<key>)
  → webhook → API Gateway → Orchestrator (Lambda, Python)
      ...
      → dedup on sonar_hash; invoke_agent(TicketPayload)
  → Coding Agent (AgentCore managed harness)
      → git clone → checkout base_branch → create fix_branch
      → kiro-cli headless (prompt = full ticket description) → run tests → open PR
...
The orchestrator owns Jira (read-only, from the webhook). The agent owns GitHub + kiro. The only thing crossing the boundary is a JSON `TicketPayload`. The `sonar-hash` label is the idempotency key — one finding, one PR.
